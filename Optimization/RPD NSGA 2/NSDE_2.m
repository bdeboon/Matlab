...
while generation < 300 %Run for 1000 Generations
...
[M1s(:,1),I1] = sort(M1(:,1),1); %take the vector and sort each of its rows 
[M2s(:,1),I2] = sort(M1(:,end-1),1); %I is needed to unsort 

%Endpoints get a value of 0 - MIGHT NEED TO BE SOMETHING DIFFERENT 
M1s(1,2) = 10000000000; 
M2s(1,2) = 10000000000;

M1s(end,2) = 10000000000;
M2s(end,2) = 10000000000;

%Calculate crawding of points between the end points 
for cc = 2:m-2
    M1s(cc,2) = M1s(cc+1,1) - M1s(cc-1,1);
    M2s(cc,2) = M2s(cc+1,1) - M2s(cc-1,1);
end

%Unsort the arrerys - get the crowding distance
M1t(I1)=M1s(:,2);
M2t(I2)=M2s(:,2);


I = [M1t;M2t]'; % Format before return 
